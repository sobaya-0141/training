...
**小規模（~30行以下）**: 自己レビューのみ。Phase 1 の差分を読んで問題点を列挙する。

**中〜大規模（31行以上）**: `code-reviewer` サブエージェントが利用できる場合は委譲する。利用できない場合は自己レビューで代替する。

```
# code-reviewer が利用できる場合
...
**code-reviewer が利用できない場合の代替手順:**
1. Phase 1 で取得した diff を自分で精読する
2. 以下の観点でチェックする:
   - ロジックの誤り・境界値の見落とし
   - 既存コードとの整合性・命名規約
   - 不要なコード・デバッグ用コードの残留
3. 問題があればそのまま修正し、Phase 1 からやり直す
